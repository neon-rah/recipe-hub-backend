...
public interface LikeRepository extends JpaRepository<Like, Integer> {
    ...
    List<Like> findAllByRecipeRecipeId(int recipeId);

    // Trouver tous les likes d'un utilisateur
    List<Like> findAllByUserUserId(UUID userId);
    ...
    Optional<Like> findByUserUserIdAndRecipeRecipeId(UUID userId, int recipeId);

    // Supprimer un like d'un utilisateur sur une recette
    void deleteByUserUserIdAndRecipeRecipeId(UUID userId, int recipeId);

    // Compter le nombre de likes d'une recette
    int countByRecipeRecipeId(int recipeId);
}
